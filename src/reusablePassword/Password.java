...
public class Password 
{
	...
	public void set(String pswd) throws Exception
	{
		...
		if (!pswdHistory.contains(encryptPswd)) // if pswd not in recently used list
		{
			...
			{
				pswdHistory.remove(0); // remove first, oldest, pswd from list
			}
	...
	private String encrypt(String pswd)
	{
		...
		midpoint = pswdSize/2;
		encryptPswd = new StringBuffer(pswd.substring(midpoint) + pswd.substring(0, midpoint)); // get last half of pswd and concatenate with first half
